...
function PremiumDetails() {
  ...
  const { data: premiums = [] , isLoading } = useQuery({
    queryKey: ["premium"],
    queryFn: async () => {
      const res = await axiosPublic.get(`/premiums/${id}`);
      return res.data;
    },
  });
  ...
  const {data : similar = [] ,  } = useQuery({
    ...
    enabled : !isLoading,
    queryFn : async() => {
      const res = await axiosPublic.post(`/biodatas/for-gender?genderType=${premiums.genderType}`)
      return res.data
    }
  })
  ...
  return (
    <div className="font-bannerFont ">
      ...
      <div className="my-16 px-5">
        ...
        {
           similar && similar?.map(premium =>  <div key={premium?._id} className='border font-bannerFont rounded-xl bg-white'>
